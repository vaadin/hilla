...
You are working on the `claude/hilla-annotation-analysis-ssSR2` branch of a Hilla fork repository.

### Context

A previous session implemented feature toggles in `application.properties` for three Hilla features: file-router, auto-crud, and vaadin-ui. All documentation is in `changes/01/`. Read these files first:

1. `changes/01/specification.md` - Full project context and task description
2. `changes/01/changenotes.md` - What was changed and why
3. `changes/01/howtotest.md` - Detailed test instructions
4. `changes/01/futuresuggestions.md` - Future improvement ideas
...
3. Run `mvn clean install -DskipTests` to build all Java packages
4. Fix any compilation errors in the changed files

#### Phase 2: Run Unit Tests

1. Run the Java unit test:
   ```bash
...
2. Verify that `HillaFeatureProperties.fromBaseDir()` correctly reads properties files
...
| File | Purpose |
|---|---|
| `packages/java/engine-core/src/main/java/com/vaadin/hilla/engine/HillaFeatureProperties.java` | New: reads feature toggles from application.properties |
| `packages/java/engine-core/src/test/java/com/vaadin/hilla/engine/HillaFeaturePropertiesTest.java` | New: unit tests |
...
Work on branch: `claude/hilla-annotation-analysis-ssSR2`
