...
it covers the client, the endpoints, the barrel, the types they refer to and the
models a form binds those types through; the unions of a polymorphic type, push
and signals still come from the Node generator, which is what the pipeline
runs.
