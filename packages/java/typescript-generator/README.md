...
`Generation` into those files, so what is left before the pipeline can run on
it is calling that. What is left of the Node generator after that is the file
...
Every case is also generated by the writers in Java, and the files they write
are compared against the same snapshots by their location, which is how far
the Java side has come: what is in each file is verified separately, against
the same classes, by the tests of the writers.
