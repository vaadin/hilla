...
const AnnotationTestEntityModel = m
  ...
  .property("oneToManyWithTargetEntity", m.meta(m.optional(m.array(m.optional(NestedEntityModel))), { annotations: [{ jvmType: "jakarta.persistence.OneToMany", attributes: { targetEntity: { jvmType: "com.vaadin.hilla.parser.plugins.model.annotations.AnnotationsEndpoint$NestedEntity" } } }], jvmType: "java.util.List" }))
  ...
  .property("manyToManyWithCascade", m.meta(m.optional(m.array(m.optional(NestedEntityModel))), { annotations: [{ jvmType: "jakarta.persistence.ManyToMany", attributes: { cascade: ["PERSIST", "MERGE"] } }], jvmType: "java.util.List" }))
